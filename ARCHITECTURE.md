...
**Version:** 1.3.1-alpha  
**Last Updated:** 2026-05-30 (Blockchain Audit Ledger + AI/ML Real + Ed25519 Signatures + Secure Protocol)  
**License:** GPL-3.0
...
2. [System Overview](#2-system-overview)
 3. [Storage Engine Architecture](#3-storage-engine-architecture)
    - 3.1 [StorageEngine Trait](#31-storageengine-trait)
    - 3.2 [Columnar Engine](#32-columnar-engine)
    - 3.3 [Vector Engine](#33-vector-engine)
      - 3.3.1 [HNSW Index](#331-hnsw-index)
      - 3.3.2 [IVF Index](#332-ivf-index)
      - 3.3.3 [Payload Filter Engine](#333-payload-filter-engine)
      - 3.3.4 [Scoring & Fusion](#334-scoring--fusion)
      - 3.3.5 [Quantization (Compression)](#335-quantization-compression)
      - 3.3.6 [Metrics & Observability](#336-metrics--observability)
      - 3.3.7 [Collection Configuration](#337-collection-configuration)
    - 3.4 [Document Engine](#34-document-engine)
    - 3.5 [Relational Engine](#35-relational-engine)
    - 3.6 [Storage Engine Comparison Matrix](#36-storage-engine-comparison-matrix)
...
8. [Cluster and Distributed Systems](#8-cluster-and-distributed-systems)
 ...
 9. [AI/ML Engine](#9-aiml-engine)
    - 9.1 [Real ML Algorithms](#91-real-ml-algorithms)
    - 9.2 [RAG Subsystem](#92-rag-subsystem)
    - 9.3 [Predictive Analytics & Vector Profiling](#93-predictive-analytics--vector-profiling)
10. [Blockchain Audit Ledger](#10-blockchain-audit-ledger)
    - 10.1 [Immutable Append-Only Log](#101-immutable-append-only-log)
    - 10.2 [SHA-256 Merkle Tree](#102-sha-256-merkle-tree)
    - 10.3 [Tamper Detection](#103-tamper-detection)
    - 10.4 [Namespace and Transaction Indexing](#104-namespace-and-transaction-indexing)
11. [Transaction Signing with Ed25519](#11-transaction-signing-with-ed25519)
    - 11.1 [Signing Process](#111-signing-process)
    - 11.2 [Verification and Non-Repudiation](#112-verification-and-non-repudiation)
12. [Secure Communication Protocol](#12-secure-communication-protocol)
    - 12.1 [Encryption (AES-256-GCM)](#121-encryption-aes-256-gcm)
    - 12.2 [Authentication (Ed25519 + HMAC)](#122-authentication-ed25519--hmac)
    - 12.3 [Trust Establishment (X.509)](#123-trust-establishment-x509)
    - 12.4 [Distributed Journaling](#124-distributed-journaling)
13. [Security and Cryptography](#13-security-and-cryptography)
14. [Caching and Optimization](#14-caching-and-optimization)
15. [Transactions and Recovery](#15-transactions-and-recovery)
16. [Drivers and Clients](#16-drivers-and-clients)
17. [Performance Characteristics](#17-performance-characteristics)
18. [Deployment Scenarios](#18-deployment-scenarios)
19. [Fault Tolerance and Recovery](#19-fault-tolerance-and-recovery)
...
The vector engine is a professional-grade vector database subsystem inside PrimusDB, purpose-built for high-dimensional similarity search, Approximate Nearest Neighbor (ANN) retrieval, payload filtering, score fusion, vector compression, RAG pipelines, and predictive analytics. It exposes the standard `StorageEngine` trait (insert/select/update/delete/analyze/create_table/drop_table/truncate_table/table_info) plus engine-specific extensions accessible via `as_any()` downcasting.

### 3.3.1 HNSW Index

The Hierarchical Navigable Small World (HNSW) implementation is a multi-layer graph index supporting incremental insert and search.

**Algorithm:**
- Each vector is assigned a random level `l = floor(-ln(rand()) × mL)` capped at `max_level`
- Level 0 contains all vectors; higher levels contain exponentially fewer
- Insert: greedily traverse from top layer to layer `l+1` to find entry point, then insert connections at layer `l` down to 0 using the `M`-nearest neighbors as bidirectional links
- Search: greedy traverse from top layer to layer 1 (single best), then at layer 0 expand candidates up to `ef` with a min-heap priority queue processing closest candidates first

**Key parameters:**
| Parameter | Default | Description |
|-----------|---------|-------------|
| `M` | 16 | Number of bidirectional links per node |
| `M_max` | 32 | Max links per node (higher layers) |
| `ef_construction` | 200 | Dynamic candidate list size during insert |
| `max_level` | 5 | Maximum number of layers |

**Search guarantees:**
- Recall ≥ 95% vs brute-force for ef ≥ 100 on uniform data
- Sub-millisecond latency for 10K–100K vectors
- O(log N) search complexity theory, O(ef × M) in practice

### 3.3.2 IVF Index

Inverted File Index partitions the vector space into `nlist` clusters via K-means++, assigns each vector to its nearest centroid, and stores per-centroid inverted lists. Search probes `nprobe` nearest centroids and only scans their lists.

**Algorithm flow:**
1. **Build**: K-means++ with Forgy seeding → cluster assignment → per-cluster inverted lists
2. **Search**: Compute distances to all `nlist` centroids → pick `nprobe` closest → scan all vectors in those clusters → return top-k
3. **Fallback**: If `nprobe ≥ nlist`, performs brute-force full scan with automatic detection

**Key parameters:**
| Parameter | Default | Description |
|-----------|---------|-------------|
| `nlist` | 100 | Number of centroids/clusters |
| `nprobe` | 10 | Number of clusters to probe during search |

### 3.3.3 Payload Filter Engine

The payload filter engine provides a composable condition system for filtering vector results by associated metadata before/after similarity scoring.

**Condition types:**

| Condition | JSON Form | Behavior |
|-----------|-----------|----------|
| `Eq` | `{"eq": {"field": "color", "value": "red"}}` | Exact match |
| `Ne` | `{"ne": {"field": "color", "value": "blue"}}` | Not equal |
| `Gt` | `{"gt": {"field": "price", "value": 100}}` | Greater than (numeric) |
| `Gte` | `{"gte": {"field": "price", "value": 100}}` | Greater than or equal |
| `Lt` | `{"lt": {"field": "price", "value": 200}}` | Less than |
| `Lte` | `{"lte": {"field": "price", "value": 200}}` | Less than or equal |
| `In` | `{"in": {"field": "tags", "value": ["a","b"]}}` | Value in list |
| `Nin` | `{"nin": {"field": "tags", "value": ["c"]}}` | Value not in list |
| `Exists` | `{"exists": {"field": "email"}}` | Field exists (non-null) |
| `Regex` | `{"regex": {"field": "name", "pattern": "^A.*"}}` | Regex match |
| `And` | `{"and": [cond1, cond2]}` | All conditions must match |
| `Or` | `{"or": [cond1, cond2]}` | Any condition must match |
| `Not` | `{"not": cond}` | Negation of condition |

Filters are parsed from the conditions map inside `Query::conditions` (skipping the reserved `"query_vector"` key) and applied during `select()` with `payload_filter` parameter.

### 3.3.4 Scoring & Fusion

The scoring engine enables multiple score combination strategies for hybrid search (e.g., combining HNSW + IVF results or multi-field similarity).

**Scoring modes:**
| Mode | Formula | Use Case |
|------|---------|----------|
| `Raw` | `score = raw_distance` | Direct distance output |
| `Normalized` | `score = 1.0 / (1.0 + raw_distance)` | [0, 1] bounded scores |
| `RRF` | `score = Σ(1 / (k + rank_i))` | Reciprocal Rank Fusion — combines multiple ranked lists |
| `Weighted` | `score = Σ(w_i × s_i) / Σ(w_i)` | Weighted arithmetic mean of scores |

**RRF Fusion** handles multiple ranked result lists with configurable constant `k` (default 60). Results missing from a list get zero contribution. **Weighted Fusion** accepts per-list weights. Both handle empty lists gracefully.

### 3.3.5 Quantization (Compression)

Two quantization schemes reduce memory footprint for large-scale deployments.

| Scheme | Output | Memory Reduction | Description |
|--------|--------|-----------------|-------------|
| **ScalarQuantizer (SQ8)** | `u8` per dimension | 4× (32→8 bits) | Linearly maps [min, max] → [0, 255] per dimension. Round-trip via dequantization. |
| **BinaryQuantizer (BQ)** | 1 bit per dimension | 32× | Threshold at median → 0/1 bits. Uses Hamming distance for similarity. |

### 3.3.6 Metrics & Observability

The `VectorMetrics` struct tracks per-query and cumulative statistics:

| Metric | Type | Description |
|--------|------|-------------|
| `queries_total` | u64 | Total queries executed |
| `vectors_total` | u64 | Total vectors stored |
| `deleted_total` | u64 | Total vectors deleted |
| `cache_hits` | u64 | In-memory vector cache hits |
| `cache_misses` | u64 | In-memory vector cache misses |
| `query_latency_ms` | Histogram | Per-query latency distribution (buckets: 1, 5, 10, 25, 50, 100, 250, 500, 1000 ms) |
| `index_build_time_ms` | u64 | Cumulative index build time |

Metrics are updated atomically and exposed via `get_metrics()`. The latency histogram supports percentile estimation (P50, P95, P99).

### 3.3.7 Collection Configuration

`CollectionConfig` stores per-table configuration persisted alongside data:

```rust
pub struct CollectionConfig {
    pub dimensions: usize,         // Vector dimensionality
    pub metric: DistanceMetric,    // Cosine | Euclidean | DotProduct | Manhattan | Hamming
    pub index_method: IndexMethod, // BruteForce | HNSW | IVF
    pub quantization: Option<QuantizationConfig>, // Optional SQ8/BQ
    pub scoring_mode: ScoringMode, // Raw | Normalized | RRF | Weighted
    pub hnsw_params: Option<HnswParams>,
    pub ivf_params: Option<IvfParams>,
}
```

### 3.3.8 Integration with StorageEngine Trait

The `VectorEngine` implements all `StorageEngine` trait methods:
- **insert** — stores vector + payload, updates HNSW/IVF indices incrementally
- **select** — performs ANN or brute-force search with optional payload filter and scoring mode
- **update** — scans sled tree, applies JSON merge to matching records
- **delete** — removes from sled and HNSW/IVF indices
- **analyze** — returns record count, field frequency, dimension, metric, index type
- **table_info** — returns `TableInfo` with real `row_count`
- **create_table** / **drop_table** / **truncate_table** — full lifecycle with sled persistence

Engine-specific APIs accessed via `as_any()`:
- `build_hnsw_index(table, params)` — rebuild HNSW from persisted data
- `build_ivf_index(table, params)` — rebuild IVF from persisted data
- `get_metrics()` — retrieve `VectorMetrics` snapshot
- `vector_search(table, query, k, filters, scoring_mode)` — direct search bypass

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    VECTOR ENGINE — COMPLETE ARCHITECTURE                    │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│                          StorageEngine Trait API                             │
│  insert()  select()  update()  delete()  analyze()  table_info()           │
│  create_table()  drop_table()  truncate_table()  as_any()                   │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                    ┌───────────────┼───────────────┐
                    ▼               ▼               ▼
        ┌───────────────────┐ ┌───────────┐ ┌───────────────────┐
        │   ANN Indexes    │ │ Payload  │ │   Scoring         │
        │                   │ │ Filter   │ │   Engine          │
        │ ┌─────┐ ┌─────┐  │ │ Engine   │ │                   │
        │ │HNSW │ │ IVF │  │ │          │ │ Raw / Normalized  │
        │ │Graph│ │ In- │  │ │ Eq / Ne  │ │ RRF / Weighted    │
        │ │     │ │ vert│  │ │ Gt/Lt/.. │ │                   │
        │ │[M,  │ │ [n- │  │ │ And/Or/  │ └───────────────────┘
        │ │ ef] │ │ list│  │ │ Not      │
        │ └─────┘ └─────┘  │ │ Regex    │
        └───────────────────┘ └──────────┘
                    ...
                    ▼               ▼               ▼
        ┌───────────────────┐ ┌───────────┐ ┌───────────────────┐
        │   Quantization   │ │  Metrics  │ │   RAG Subsystem   │
        │                   │ │ & Obser- │ │                   │
        │ SQ8 (f32 → u8)   │ │ vability  │ │ Chunk Document    │
        │ BQ (f32 → 1 bit) │ │          │ │ (Fixed/Recursive/ │
        │                   │ │ Latency  │ │  SlidingWindow)   │
        │                   │ │ Histogram│ │ Retrieve Similar  │
        └───────────────────┘ └──────────┘ └───────────────────┘
                                    │
                                    ▼
        ┌─────────────────────────────────────────────────────┐
        │                 Sled Persistence                     │
        │  vectors:{table} → HashMap<String, VectorRecord>    │
        │  configs:{table} → CollectionConfig                  │
        │  hnsw_indices / ivf_indices → Mutex<HashMap<...>>   │
        └─────────────────────────────────────────────────────┘
```

### 3.3.9 Distance Metrics

| Metric | Formula | Range | Use Case |
|--------|---------|-------|----------|
| Cosine | `1 - (A·B)/(|A||B|)` | [0, 2] | Text/document embeddings |
| Euclidean | `√(Σ(Ai-Bi)²)` | [0, ∞) | General purpose |
| DotProduct | `Σ(Ai×Bi)` | (-∞, ∞) | Normalized embeddings |
| Manhattan | `Σ|Ai-Bi|` | [0, ∞) | High-dimensional sparse |
| Hamming | `popcount(A⊕B)` | [0, dims] | Binary vectors |
...
PrimusDB's Key-Value engine implements a CouchDB-compatible API providing document storage with MVCC versioning through `_id`/`_rev` tokens. It is backed by the sled embedded database for persistence and supports Mango-style queries, bulk operations, and index management.

### Design Goals

- **CouchDB-compatible wire protocol** so existing CouchDB client tools can connect
- **Embedded persistence** via sled (no external database daemon)
- **Optimistic concurrency** using revision tokens (_rev)
- **Schema-flexible documents** stored as JSON
- **Selector-based queries** with 10 operators ($eq, $gt, $gte, $lt, $lte, $ne, $in, $nin, $exists, $type)
...
pub struct KvDocument {
    pub _id: String,
    pub _rev: Option<String>,           // "1-{hash}", "2-{hash}", ...
    pub value: serde_json::Value,        // user data
    pub created_at: Option<String>,
    pub updated_at: Option<String>,
    pub deleted: bool,                   // tombstone for replication
}

pub struct KvDatabase {
    pub name: String,
    pub documents: Arc<RwLock<HashMap<String, KvDocument>>>,
    pub indexes: Arc<RwLock<HashMap<String, KvIndex>>>,
    pub sequence: Arc<RwLock<u64>>,
    pub attachments: Arc<RwLock<HashMap<String, HashMap<String, KvAttachment>>>>,
}

pub struct KeyValueEngine {
    pub config: PrimusDBConfig,
    pub db: sled::Db,                                          // persistent tree store
    pub databases: Arc<RwLock<HashMap<String, KvDatabase>>>,
    pub encrypted_databases: Arc<RwLock<HashMap<String, bool>>>,
}
```

### Revision Tokens

The revision format is `{generation}-{hash}` (e.g., `1-a1b2c3d4`, `2-e5f6g7h8`):

1. **Generation** — auto-incrementing integer
2. **Hash** — derived from timestamp XORed with a random value

On every `put_document`, the engine reads the existing `_rev`, increments the generation, and produces a new hash. Deletes also produce a tombstone with an incremented revision.

### Persistence

All databases are stored in sled trees under `{data_dir}/keyvalue/`. Each database corresponds to one sled tree named by the database name. On engine startup, `load_databases()` reads all sled trees into in-memory `KvDatabase` structures.

### Mango Queries

The `find()` method evaluates selector expressions against `KvDocument.value` using a recursive `matches_selector()` function. Supported operators:
...
| `$type`  | JSON type check ("string", "number", "boolean", "null") |

Results can be sorted by field with `asc`/`desc` direction.

### Public API

**Database management:**
- `create_database(name)` — create a new database
- `delete_database(name)` — remove a database and its sled tree
- `list_databases()` — list all database names
- `get_db_info(name)` — return metadata (doc_count, sequence, etc.)

**Document CRUD:**
- `get_document(db, doc_id)` — retrieve a document (error if deleted or missing)
- `put_document(db, doc_id, value)` — create or update (auto _rev generation)
- `delete_document(db, doc_id, rev)` — soft-delete (tombstone), requires current _rev

**Bulk operations:**
- `bulk_docs(db, docs, all_or_nothing)` — batch insert/update with conflict detection
- `all_docs(db, include_docs, limit, skip)` — paginated document listing

**Query and indexing:**
- `find(db, request)` — Mango selector query
- `create_index(db, name, fields, selector)` — create a named index
- `list_indexes(db)` — list all indexes on a database

**Maintenance:**
- `ensure_full_commit(db)` — flush sled tree to disk
- `compact(db)` — log compaction request
- `get_revision_limit(db)` / `set_revision_limit(db, limit)` — revision limit control

## 4.3 REST API Endpoints

| Method | Route | Description |
|--------|-------|-------------|
| GET    | `/api/v1/kv/:db` | Database metadata |
| PUT    | `/api/v1/kv/:db` | Create database |
| DELETE | `/api/v1/kv/:db` | Delete database |
| GET    | `/api/v1/kv/:db/:docid` | Get document |
| PUT    | `/api/v1/kv/:db/:docid` | Create or update document |
| POST   | `/api/v1/kv/:db/:docid` | Update document (upsert) |
| DELETE | `/api/v1/kv/:db/:docid?rev={rev}` | Delete document |
| GET    | `/api/v1/kv/:db/_all_docs` | List documents with pagination |
| POST   | `/api/v1/kv/:db/_find` | Mango query |
| GET    | `/api/v1/kv/:db/_index` | List indexes |
| POST   | `/api/v1/kv/:db/_index` | Create index |
| POST   | `/api/v1/kv/:db/_bulk_docs` | Bulk document operations |
| POST   | `/api/v1/kv/:db/_compact` | Compact database |
| POST   | `/api/v1/kv/:db/_ensure_full_commit` | Flush to disk |
| GET    | `/api/v1/kv/:db/_rev_limit` | Get revision limit |
| PUT    | `/api/v1/kv/:db/_rev_limit` | Set revision limit |

## 4.4 Cross-Cutting Integration

The KV engine is integrated with the following system-wide services:

### 4.4.1 StorageEngine Trait

`KeyValueEngine` implements the `StorageEngine` trait (defined in `src/storage/mod.rs`), making it available through the unified engine map (`storage_engines: HashMap<StorageType, Arc<dyn StorageEngine>>`) used by the consensus state machine, transaction manager, and UQL executor. The trait implementation maps:
- `create_table` → `create_database`
- `insert` → `put_document` (with auto-ID generation when `_id` is missing)
- `select` → `find` with optional conditions matching
- `update` → scan-and-update with selector matching
- `delete` → scan-and-delete with tombstone creation
- `table_info` → `get_db_info`

### 4.4.2 Consensus Integration

The consensus state machine (`src/consensus/state_machine.rs`) already parses `"KeyValue"` as a recognized `StorageType`. KV operations submitted through the consensus pipeline (via `OperationType::Insert`, `Update`, `Delete`, `Create`, `Drop`) are dispatched to `KeyValueEngine` through its `StorageEngine` implementation. KV-specific operations (`_find`, `_index`, `_bulk_docs`, `_compact`) are query-time operations that bypass consensus — they execute directly on the local KV engine via the REST handlers.

### 4.4.3 Encryption

The `encrypted_databases` map is no longer dead code. `KeyValueEngine` now holds an `Option<Arc<Mutex<CryptoManager>>>`. When encryption is enabled for a database via `enable_database_encryption()`:
- **put_document**: the document's `value` field is encrypted with AES-256-GCM before storage. The encrypted payload (nonce, ciphertext, tag, key_id, algorithm) is serialized as a JSON object with `__encrypted__: true`.
- **get_document**: the encrypted wrapper is detected and decrypted transparently before returning to the caller.

### 4.4.4 Namespace Isolation

All 16 KV REST handlers accept an optional `?namespace=` query parameter. When namespaces are enabled and a namespace is provided, the `resolve_kv_ns()` helper resolves the database name through `NamespaceController::resolve_physical_name()`, ensuring that KV databases are isolated per namespace. This is consistent with the namespace isolation used by the other storage engines.

### 4.4.5 Metrics

The Prometheus metrics system (`src/metrics.rs`) includes KV-specific counters and histograms:
- `primusdb_kv_operations_total` — total KV operations
- `primusdb_kv_operation_errors_total` — failed KV operations
- `primusdb_kv_operation_latency_seconds` — operation latency distribution
- `primusdb_kv_databases_total` — number of KV databases
- `primusdb_kv_documents_total` — total documents across all databases

Helper functions `record_kv_operation()` and `update_kv_db_gauges()` allow KV handlers and the engine to report metrics.

### 4.4.6 Federation

The `FedClusterAnnounce` message in `src/cluster/federation.rs` includes `"keyvalue"` in its capabilities list, so federated peers are aware of KV support.

### 4.4.7 Auth & Drivers

- `ResourceType::KeyValue` is available for RBAC scope definitions in `src/auth/mod.rs`.
- `DriverFeature::KeyValue` is available for driver capability advertisements in `src/drivers/mod.rs`.
- `parse_storage_type()` in `src/api/mod.rs` accepts `"kv"` and `"keyvalue"` as valid storage type strings, mapping to `StorageType::KeyValue`.

## 4.5 Limitations

- **Compaction**: `compact()` is a stub that logs the request
- **Revision limit**: `set_revision_limit()` accepts the value but always returns the stored default (1000)
- **No TTL/expiry**: documents live until explicitly deleted
- **No conflict resolution strategies**: the engine uses last-write-wins with _rev tracking; applications must handle conflicts at the client layer
...
Transaction management in PrimusDB is built on the fundamental principle that consistency should be the default, not the exception. Every operation that could affect data integrity must have clearly defined transactional semantics. The transaction manager coordinates ACID properties across all storage engines, providing atomicity and durability through write-ahead logging (WAL).

The architecture follows a **layered approach**: the transaction layer orchestrates begin/commit/rollback, captures before/after images for rollback, and delegates actual data operations to the storage engines. Storage engines themselves are transaction-unaware — they execute operations without MVCC or locking at the engine level. All transaction semantics are managed at the coordination layer.

### 5.1.2 Core Types

The transaction system is defined in `src/transaction/mod.rs` with the following core types:
...
| Method | Description |
|--------|-------------|
| `begin_transaction()` | Creates a new transaction, logs to sled-backed WAL, stores in active map |
| `commit_transaction(tx)` | Two-phase: writes journal entries, proposes to consensus engine, flushes journal on success |
| `rollback_transaction(id)` | Reads logs, reverses operations using before/after images via engine calls |
| `create_savepoint(tx_id, name)` | Creates a savepoint at current operation index |
| `rollback_to_savepoint(tx_id, name)` | Rolls back to a named savepoint |

### 5.1.4 Storage Persistence

Transaction data is persisted using sled embedded database:

- **Journal** (`{data_dir}/transactions/journal/`): WAL entries (`JournalEntry`) with LSN, transaction ID, operation, timestamps, and checksums
- **Transaction Log** (`{data_dir}/transactions/logs/`): Sequential log of all transaction operations for rollback recovery

Both use sled trees with ordered keys for efficient prefix scanning.

### 5.1.5 Integration with ExecuteQuery

Every query through `PrimusDB::execute_query()` is automatically wrapped in a transaction:
...
execute_query(query)
  ...
  │
  ├─ Capture before_image (for update/delete: SELECT before mutation)
  ├─ engine.insert/update/delete(... &transaction ...)
  ├─ Record after_image + rollback_data in transaction.operations
  │
  ├─ transaction_manager.commit_transaction(tx)
  │     ├─ Write journal entries for each operation
  │     ├─ Propose to consensus engine
  │     └─ Flush journal on success
  │
  └─ Return QueryResult
```

On rollback, the TransactionManager iterates operations in reverse order and uses before/after images to undo:
- **INSERT**: Delete using rollback_data as condition
- **UPDATE**: Restore original records from before_image
- **DELETE**: Re-insert records from rollback_data
...
| POST | `/api/v1/transaction/{id}/commit` | Commit and persist all queued operations |
| POST | `/api/v1/transaction/{id}/rollback` | Rollback all queued operations |

### 5.1.7 Limitations

- **No MVCC at engine level**: storage engines do not use the transaction parameter for MVCC visibility — isolation is managed entirely through the coordination layer
- **No distributed locking**: reads are not isolated with row/table locks; write conflicts are detected during consensus
- **Savepoints**: `create_savepoint` and `rollback_to_savepoint` are stubs
- **Consensus state machine**: creates a synthetic `dummy_tx` during block replay instead of going through the TransactionManager (valid for replay, but inconsistent with the live code path)
- **No configurable timeouts**: `timeout_ms` is hardcoded to 30000ms in `begin_transaction`
- **No prepared transactions**: `Prepared` status exists but two-phase prepare is not exposed externally
...
| Java | primusdb-java | Official (213 tests, PreparedStatement + Batch ✓) |
| C#/.NET | primusdb-dotnet | Official |
| Ruby | primusdb-ruby | Official |
| PHP | primusdb-php | Community |

## 14.2 Driver Feature Parity

| Feature | Rust | Python | Java | Node.js | Ruby |
|---------|------|--------|------|---------|------|
| Async operations | ✓ | ✓ | ✗ | ✓ | ✓ |
| Connection pooling | ✓ | ✗ | ✓ | ✓ | ✗ |
| Prepared statements | ✗ | ✓ | ✓ (setInt/String/Double + executeBatch) | ✗ | ✗ |
...
9. **AI/ML Engine** - Real ML algorithms (LinearRegression, LogisticRegression, K-Means++, TimeSeries, AnomalyDetection, Forecast, PatternAnalysis), RAG subsystem, vector profiling
10. **Blockchain Audit Ledger** - Immutable append-only log, SHA-256 Merkle tree, tamper detection, namespace indexing
11. **Transaction Signing with Ed25519** - Cryptographic signing, signature verification, non-repudiation
12. **Secure Communication Protocol** - AES-256-GCM encryption, Ed25519 + HMAC authentication, X.509 trust, distributed journaling
13. **Security and Cryptography** - Authentication, encryption, access control
14. **Caching and Optimization** - Multi-level cache, query optimization
15. **Transactions and Recovery** - Recovery procedures, WAL replay
16. **Drivers and Clients** - Official driver support (Rust, Python, Java, Node.js, Ruby)
17. **Performance Characteristics** - Benchmarks and metrics
18. **Deployment Scenarios** - Deployment options
19. **Fault Tolerance and Recovery** - Failure detection

---

## 17. Blockchain Audit Ledger

PrimusDB includes a tamper-evident blockchain audit ledger that provides an immutable, append-only record of all transactions. This is not a cryptocurrency or smart contract platform — it is a **cryptographic audit trail** designed for enterprise compliance, forensics, and integrity verification.

### 17.1 Architecture

The audit ledger is built on three core components:

- **Block**: Each block contains a set of transactions, a timestamp, and the SHA-256 hash of the previous block, forming an unbroken hash chain
- **Merkle Tree**: Transactions within each block are hashed into a binary Merkle tree, enabling efficient inclusion proofs without revealing all transactions
- **Ledger Store**: Sled-backed persistent storage provides durability across restarts, with indexes by namespace and transaction ID

```
Block N-1                          Block N                           Block N+1
┌──────────────────┐               ┌──────────────────┐              ┌──────────────────┐
│ Prev Hash: ...   │───hash──→     │ Prev Hash: ✓     │───hash──→    │ Prev Hash: ✓     │
│ Merkle Root: ... │               │ Merkle Root: ... │              │ Merkle Root: ... │
│ Tx 1 │ Tx 2 │... │               │ Tx A │ Tx B │... │              │ Tx X │ Tx Y │... │
└──────────────────┘               └──────────────────┘              └──────────────────┘
```
...
pub struct AuditLedger {
    db: sled::Db,
    blocks: sled::Tree,     // block_hash → serialized Block
    indices: sled::Tree,    // index key → block_hash
}

impl AuditLedger {
    /// Append a new block to the chain
    pub fn append_block(&self, transactions: Vec<Transaction>) -> Result<Block>;
    
    /// Verify the integrity of the entire chain
    pub fn verify_chain(&self) -> Result<TamperReport>;
    
    /// Look up a transaction across all blocks
...
### 17.3 Tamper Detection

The `verify_chain()` method walks every block from genesis to head, recomputing hashes and comparing against stored values. Any discrepancy produces a `TamperReport` detailing:

- Which block was tampered
- The expected vs actual hash
- Whether the Merkle root is consistent with transaction hashes
- Whether the chain link to the next block is broken

This enables automated integrity monitoring and forensic investigation.

### 17.4 Use Cases

- **Compliance Audits**: Prove that data has not been altered since recording
- **Forensic Analysis**: Trace the history of specific records across time
- **Integrity Monitoring**: Automated detection of unauthorized modifications
- **Regulatory Reporting**: Provide tamper-evident transaction history for regulators

## 18. Transaction Signing with Ed25519

Every transaction in PrimusDB can be cryptographically signed using Ed25519 digital signatures, providing non-repudiation and integrity verification.

### 18.1 Signing Process

The `Transaction` struct includes two optional fields:

```rust
pub struct Transaction {
    // ... existing fields ...
    pub signature: Option<Vec<u8>>,   // Ed25519 signature
    pub public_key: Option<Vec<u8>>,  // Signer's public key
}
```

Signing uses `ed25519-dalek`:
1. Serialize the transaction payload to canonical JSON (excluding the `signature` field to enable verification)
2. Sign the serialized bytes with the signer's Ed25519 secret key
3. Store the resulting signature + public key in the transaction
...
`verify_signature()` recomputes the canonical JSON from the payload, then uses `ed25519-dalek::PublicKey.verify_strict()` to validate the signature. This prevents forgery, replay, and tampering.

### 18.3 Integration

Signed transactions are stored in the blockchain audit ledger, creating a complete chain of cryptographically verifiable operations. This provides:
- **Non-Repudiation**: The signer cannot deny having issued the transaction
- **Integrity**: Any modification invalidates the signature
- **Auditability**: Every operation can be traced to its author

## 19. Secure Communication Protocol

PrimusDB includes a full-featured secure communication protocol for inter-node and client-node communication, providing encryption, authentication, and integrity at the application layer.

### 19.1 Protocol Stack

| Layer | Mechanism | Purpose |
|-------|-----------|---------|
| Encryption | AES-256-GCM (ring) | Confidentiality of all data in transit |
| Signing | Ed25519 (ed25519-dalek) | Authenticity and non-repudiation |
| Integrity | HMAC-SHA256 | Tamper detection on messages |
| Trust | X.509 certificates | Identity verification and PKI |
| Journaling | Append-only log | Distributed audit trail |

### 19.2 Encryption (AES-256-GCM)

Each message is encrypted with AES-256-GCM using `ring::aead`. A random 12-byte nonce is generated per message, prefixed to the ciphertext. GCM provides authenticated encryption — any tampering with the ciphertext is detected during decryption.
...
### 19.3 Authentication (Ed25519 + HMAC)

Messages carry dual authentication:
- **Ed25519 signature**: Proves the message originated from a specific trusted node
- **HMAC-SHA256**: Provides additional integrity verification with a shared secret

### 19.4 Trust Establishment (X.509)

Nodes establish trust through certificate verification:
- Each node possesses an X.509 certificate signed by a trusted CA
- During handshake, nodes exchange certificates and verify signatures
- Compromised certificates can be revoked via a CRL-style revocation list
- `TrustManager` maintains the set of trusted certificates and revoked certificates

### 19.5 Distributed Journaling

The protocol includes an append-only journal for recording all communication events:
- Entry buffering with configurable flush intervals
- Sled-backed persistence for crash recovery
