...
The Resource Governor subsystem prevents runaway queries by tracking executions against configurable policies. It supports 14 workload types (SQL, vector, AI/ML, graph, CDC, backup, restore, migration, cluster ops, protocol, UDF, stored procedures, plugins, FFI) with 4 enforcement actions (monitor, warn, throttle, block).

### GET /api/v1/governor/status
Get governor status (enabled/disabled, active execution count).
...
### GET /api/v1/governor/policies
List all configured resource policies.
  ...
  "data": [
    {
      ...
      "action": "monitor",
      "max_memory_mb": 2048,
      "max_execution_steps": 10000000
    }
  ]
}
```

### GET /api/v1/governor/metrics
Get a metrics snapshot from active executions.
...
### GET /api/v1/governor/violations
...
### GET /api/v1/governor/executions
...
### POST /api/v1/governor/executions/start
...
### POST /api/v1/governor/executions/{id}/finish
Finish a tracked execution and release its resources.
...
### POST /api/v1/governor/executions/{id}/check
Check whether a resource usage value exceeds the execution's policy limits.

**Request:**
```json
{
  "check_type": "max_memory_mb",
  "value": 1024
}
```
...
```json
{
  ...
  "data": {
    "action": "block",
    "message": "Resource limit exceeded: max_memory_mb (limit: 2048, usage: 5120). Action: blocked"
  }
}
```

### POST /api/v1/governor/policies/update
...
```json
{
  "name": "my-policy",
  "limits": {
    "cpu": { "max_cpu_time_ms": 60000, "max_execution_steps": 1000000 },
    "memory": { "max_memory_mb": 1024 },
    "query_complexity": { "max_query_complexity": 50, "max_join_count": 5 },
    "pipeline": { "max_pipeline_depth": 50 },
    "ffi": { "max_ffi_calls": 5000 },
    "aiml": { "max_training_iterations": 50000 },
    "vector": { "max_vector_candidates": 50000 },
    "graph": { "max_graph_depth": 50 },
    "migration": { "max_import_rows": 5000000 },
    "backup": { "max_backup_size": 1073741824 }
  },
  "action": "warn",
