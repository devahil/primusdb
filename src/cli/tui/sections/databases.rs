...
pub fn render_databases(frame: &mut Frame, area: Rect, app: &TuiApp) {
    ...
    if !app.connected() {
    ...
    } else if app.databases_data.is_empty() {
        lines.push(Line::from(Span::styled(
            "No databases found.",
        ...
        let count_str = format!("  {} database(s):", app.databases_data.len());
        lines.push(Line::from(Span::styled(
            count_str,
        ...
        for db in &app.databases_data {
            lines.push(Line::from(vec![
                Span::styled("  • ", Style::new().fg(Color::DarkGray)),
                Span::styled(db, Style::new().fg(Color::White)),
            ]));
        }
    }
