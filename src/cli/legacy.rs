...
pub async fn run_cli(cli: Cli) -> crate::Result<()> {
    ...
    match cli.command {
        Commands::Server { config, bind } => {
            println!("Starting PrimusDB server on {}... (legacy CLI)", bind);
            println!("Configuration: {:?}", config);
            println!("NOTE: Use `primusdb server` (new CLI) for the actual server.");
...
"#;
        ...
        Commands::Status => {
            println!("PrimusDB Status (legacy CLI):");
            ...
            println!("NOTE: Use `primusdb server status` (new CLI) for live status.");
        }
...
async fn handle_namespace_client_command(
    server: String,
